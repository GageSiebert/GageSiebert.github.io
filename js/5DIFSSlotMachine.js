...
function randomContractionMatrix(n) {
    ...
    if (maxEigenvalue > 1) {
        matrix = matrix.map(row => row.map(value => value / (maxEigenvalue + 1)));
    }
...
function generateIFSPoints(Npts,Nmaps) {
    ...
    for (var i = 0; i < Nmaps; i++) {
        maps.push({
            matrix: randomContractionMatrix(5),
    ...
    var pts = []; 
    pts.push({
        x: [Math.random(), Math.random(), Math.random(), Math.random(), Math.random()] // This is the initial point
    }); 
    //console.log(pts[0].x);
    ...
    for (var i = 0; i < Npts; i++) {
        points.push({
            x: pts[i].x[0] * width,
            y: pts[i].x[1] * height,
            color: rgbToHex([pts[i].x[2],pts[i].x[3],pts[i].x[4]])
        });
    }
